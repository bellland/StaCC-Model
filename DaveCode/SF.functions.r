...
update_datapars <- function(){  #update ba for depth model and bstat for canopy
  ...
  if(SECTION){
    ...
    for(j in 2:length(DEPTH))
	  pba[j,] <- tnorm(nspec,loba[j,],pba[j-1,],bag[j,],sqrt(pcovba[j,j]))
...
update_rint <- function(){  
  ...
  if(SECTION) {
  if (max(depth)==3){
	rave<-mean(rint[depth==1])*(SAratio[1])+mean(rint[depth==3])*(SAratio[2]+SAratio[3])} else {
    rave<-mean(rint[depth==1])*(SAratio[1])+mean(rint[depth==3])*(SAratio[2])+mean(rint[depth==5])*(SAratio[3])}
    rint<-rint/as.numeric(rave[1])
  }
  ## rescale to intercept
  if(!SECTION) rint <- rint * meana[specindex] / tapply(rint,specindex,mean)[specindex]  
  #rint
  #temporary
...
priormat['prior.B1',]   <- .90  #light
priormat['prior.B2',]   <- 250 #mmol m-2 s-1
...
inData <- function(){
  ...
  if(SECTION) {
    ...
    SAratio <- SAratio[SAratio[,'year']==year,][match(Treat,SAratio[,'Treat']),]
  	  SAratio <- matrix(SAratio, ncol=6)
...
#  ntime <- length(keeptime)
  ...
  # draw depth values
  if(SECTION) depth <- as.numeric(b2[,'Depth'])
  if(!SECTION)  {
