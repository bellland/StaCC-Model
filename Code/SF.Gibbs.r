...
#
  ...
  vgibbs[g,] <- c(sigma,verror,va,tau,beta)
  rgibbs[g,] <- aig
  if(g == gburnin) {
  ...
  print(c(sigma,verror,va,tau,beta))
  ...
  print(c("beta  ",paste(uk,sep="  ")),quote=F)
  ...
  if(g %in% kg){
    ...
    if(CAP & priormat['lobeta',1]!=priormat['hibeta',1]){
	  tK <- pcovK
	  if(min(uk) > 12) 
	    tK  <- 2*var(vgibbs[(g-40):(g-1),grep('beta',colnames(vgibbs))])
      if(min(uk) < 5)  
	    tK  <- pcovK*.5
	  if(nspec==1)
   	    if(tK>0) pcovK <- tK
...
#############
  ...
  if(g %in% saveg)
    save.image(paste("Results/",TITLE,'_',ng,'_',yrtrunc,'_TC_',TC,
	                                          '_SP_',SP,
	                                          '_flat_',FLAT,
                                              '_rand_',RAND,
                                              '_bstat_',BS,
